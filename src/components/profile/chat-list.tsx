...
//     const mergedChats = [...chatsData];

//     optimisticChats.forEach((optimisticChat) => {
//       const existingIndex = mergedChats.findIndex(
//         (chat) => chat._id === optimisticChat._id
//       );

//       if (existingIndex !== -1) {
//         // Update existing chat with optimistic data but preserve participants
//         mergedChats[existingIndex] = {
//           ...mergedChats[existingIndex],
//           lastMessage: optimisticChat.lastMessage,
//           updatedAt: optimisticChat.updatedAt,
//           // Keep the original participants data
//           participants: mergedChats[existingIndex].participants,
//         };
//       } else {
//         // For new chats, use optimistic data but try to find participants from existing chats
//         const participantInfo = getParticipantInfo(optimisticChat);
//         mergedChats.unshift({
//           ...optimisticChat,
//           participants: [{ 
...
//     return mergedChats;
//   }, [chatsData, optimisticChats, getParticipantInfo]);
...
//           // Participants will be filled from API data or existing chat
//         };

//         if (chatIndex !== -1) {
//           // Update existing chat - preserve existing participant data
...
//           };
//         } else {
//           // Add new chat - we'll get proper participant data from API refetch
...
//     };
//   }, [refetch, currentUserId, currentUserRole]);
...
//       if (chatIndex !== -1) {
//         // Update existing chat - preserve participant data
//         updatedChats[chatIndex] = {
//           ...updatedChats[chatIndex],
//           ...updatedChat,
//         };
//       } else {
//         // For new chats, we'll rely on API refetch for participant data
//         updatedChats.push(updatedChat);
//       }
...
export default function ChatList() {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (newMessage: any) => {
      ...
      const existingChat = chatsData.find((chat: any) => chat._id === newMessage.chatId);

      if (existingChat) {
        // Only update optimistic state if chat exists in API data
        setOptimisticChats((prev) => {
          const updatedChats = prev.filter(chat => chat._id !== newMessage.chatId);
